...
export function BottomTimeline() {
  ...
  // --- Drag & drop entre pistas con Pointer Events ---
  const findEdgeCandidates = (excludeClipId: string): number[] => {
    const edges: number[] = [playheadFrame];
    for (const track of project.tracks) {
      for (const c of track.clips) {
        if (c.id === excludeClipId) continue;
        edges.push(c.startFrame);
        edges.push(c.startFrame + c.durationFrames);
      }
    }
    return edges;
  };
  ...
  const handlePointerDown = (e: React.PointerEvent, clip: Clip, track: { id: string; type: TrackType }) => {
    ...
    const onMove = (moveEvent: PointerEvent) => {
      ...
      for (const edge of edgeCandidates) {
        if (Math.abs(edge - rawFrame) <= snapThresholdFrames) {
          rawFrame = edge;
          snapLineX = edge * pixelsPerFrame;
          break;
        }
        const endEdge = edge - clip.durationFrames;
        if (Math.abs(endEdge - rawFrame) <= snapThresholdFrames) {
          rawFrame = endEdge;
          snapLineX = edge * pixelsPerFrame;
          break;
        }
      ...
      // Detectar sobre qué pista está el puntero
      ...
      const updated: DragState = {
        ...state, deltaX, deltaY, hoveredTrackId, isCompatible,
        snappedFrame: Math.max(0, rawFrame), snapLineX,
      };
    ...
    const onUp = () => {
      ...
      try {
        moveClip(
          state.clipId,
          state.snappedFrame,
          state.hoveredTrackId !== state.originTrackId ? state.hoveredTrackId ?? undefined : undefined
        );
      } catch {
        setRejectedClipId(state.clipId);
        setTimeout(() => setRejectedClipId(null), 320);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 12px', borderBottom: '1px solid var(--border-subtle)' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          <span className="mono" style={{ fontSize: '12px', width: '40px', textAlign: 'center' }}>
        ...
        <div style={{ width: '90px', flexShrink: 0, borderRight: '1px solid var(--border-subtle)', paddingTop: '20px' }}>
          {project.tracks.map((track) => (
            <div key={track.id} style={{
              height: '48px', marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '6px', padding: '0 8px',
            }}>
        ...
        <div style={{ flex: 1, overflowX: 'auto', position: 'relative', padding: '0 12px' }}>
          <div style={{ position: 'relative', height: '20px', borderBottom: '1px solid var(--border-subtle)' }}>
            {Array.from({ length: markCount }).map((_, i) => (
              <div key={i} style={{
                position: 'absolute', left: `${i * framesPerMark * pixelsPerFrame}px`,
                top: 0, fontSize: '10px', color: 'var(--text-secondary)',
              }} className="mono">
                {i * secondsMarkStep}s
              </div>
            ))}
          ...
          <div style={{ position: 'relative', paddingTop: '4px' }}>
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    background: isInvalidHover
                      ? 'rgba(224,120,86,0.15)'
                      : isHoveredDropTarget
                    ...
                    return (
                      ...
                      >
                        ...
                        {selectedClipId === clip.id && !isBeingDragged && (
                          <>
                            <div onMouseDown={(e) => handleTrimStart(e, clip.id, 'start')} style={{
                              position: 'absolute', left: 0, top: 0, bottom: 0, width: '8px',
                              background: 'rgba(11,13,16,0.4)', cursor: 'ew-resize',
                            }} />
                            <div onMouseDown={(e) => handleTrimStart(e, clip.id, 'end')} style={{
            ...
            {drag?.snapLineX !== null && drag?.snapLineX !== undefined && (
              <div style={{
                position: 'absolute', top: 0, bottom: 0, left: `${drag.snapLineX}px`,
                width: '1px', background: '#fff', pointerEvents: 'none', zIndex: 15,
              }} />
            )}
            ...
            {project.tracks.length > 0 && (
              <div style={{
                ...
                left: `${playheadFrame * pixelsPerFrame}px`,
                width: '2px', background: 'var(--accent)', pointerEvents: 'none', zIndex: 10,
              }} />
            )}
          </div>
        </div>
